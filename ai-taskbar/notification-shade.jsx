...
function TaskIcon({ kind, color }) {
  if (kind === "doc") {
    return (
      <svg viewBox="0 0 64 64" width="100%" height="100%" aria-hidden="true">
        <path
          d="M14 8 h26 l12 12 v36 q0 4-4 4 h-34 q-4 0-4-4 v-44 q0-4 4-4 z"
          fill={color}
          fillOpacity="0.18"
          stroke={color}
          strokeWidth="3"
        ...
        />
        <path
          d="M20 32 h22 M20 40 h22 M20 48 h14"
          stroke={color}
          strokeWidth="3"
          strokeLinecap="round"
        />
    ...
    return (
      <svg viewBox="0 0 64 64" width="100%" height="100%" aria-hidden="true">
        <rect
          ...
          strokeWidth="3"
        />
        <circle cx="22" cy="24" r="4" fill={color} />
        <path
          d="M8 46 l14-14 l10 10 l8-6 l14 12"
          fill="none"
          stroke={color}
          strokeWidth="3"
          strokeLinejoin="round"
  ...
  if (kind === "plane") {
    return (
      <svg viewBox="0 0 64 64" width="100%" height="100%" aria-hidden="true">
        <path
          d="M6 36 l52-18 l-6 12 l-22 6 l-8 16 l-6-2 l4-12 l-12-2 z"
          fill={color}
          fillOpacity="0.18"
          stroke={color}
          strokeWidth="3"
  ...
  if (kind === "bowl") {
    return (
      <svg viewBox="0 0 64 64" width="100%" height="100%" aria-hidden="true">
        <path
          d="M6 30 h52 q0 16-14 22 h-24 q-14-6-14-22 z"
          fill={color}
          fillOpacity="0.18"
          stroke={color}
          strokeWidth="3"
        ...
        />
        <path
          d="M22 22 q0-6 6-6 q3 0 3-4 M36 22 q0-6 6-6"
          fill="none"
          stroke={color}
          strokeWidth="3"
          strokeLinecap="round"
    ...
    <div className="prog">
      ...
      <div
        ...
        style={{
          ...
          background: `linear-gradient(90deg, oklch(from ${color} l c h / 0.4), ${color})`,
        }}
      >
...
function TaskCard({ task, index, visible }) {
  const delay = visible ? index * 70 + 80 : 0;

  return (
    <div
      className={`task ${visible ? "in" : ""}`}
      style={{ transitionDelay: `${delay}ms`, "--accent": task.accent }}
    >
      <div className="task-row">
        <div className="task-ico">
          <TaskIcon kind={task.icon} color={task.accent} />
        </div>
        <div className="task-body">
          ...
          <div className="task-bar-wrap">
            <ProgressBar value={task.progress} color={task.accent} />
          </div>
...
export default function NotificationShade({ open, progress, tasks, onClose }) {
  const animationProgress = open ? 1 : progress;
  const translate = -100 + animationProgress * 100;
  const visible = animationProgress > 0.7;
  const scale = 0.985 + animationProgress * 0.015;
  const backgroundOpacity = Math.min(1, animationProgress * 1.15);
    ...
    >
      ...
      <div className="shade-content">
        <div className="shade-statusbar">
          <span>10:42</span>
          <span className="shade-hint-copy">下拉以查看更多</span>
        </div>
        ...
        <div className="shade-top">
          ...
          <div className="shade-quick">
            <div className="qchip active">
              <span
                className="qchip-dot"
                style={{ background: "oklch(0.85 0.14 200)" }}
              />
              WiFi
            </div>
            <div className="qchip">
            ...
            <div className="qchip">
              <span
                className="qchip-dot"
                style={{ background: "oklch(0.85 0.16 75)" }}
              />
              勿扰
            </div>
          </div>
          ...
          <div className="ssh-left">
            <div className="ssh-spark">
              <svg viewBox="0 0 24 24" width="38" height="38" aria-hidden="true">
                <defs>
                  <linearGradient id="taskbarSparkGradient" x1="0" y1="0" x2="1" y2="1">
                    <stop offset="0" stopColor="#9DE9FF" />
                    <stop offset="1" stopColor="#C9A6FF" />
                  </linearGradient>
                </defs>
                <path
                  d="M12 2 L14 9 L21 12 L14 15 L12 22 L10 15 L3 12 L10 9 Z"
                  fill="url(#taskbarSparkGradient)"
                />
              </svg>
            </div>
            <div>
        ...
        <div className="tasks">
          {tasks.map((task, index) => (
            <TaskCard key={task.id} task={task} index={index} visible={visible} />
          ))}
        </div>

        <div className="shade-section-head notification-head">
          <div className="ssh-title sm">通知</div>
          <div className="ssh-sub-small">2 条新通知</div>
        </div>
        <div className="notif-list">
          <div
            className={`notif ${visible ? "in" : ""}`}
            style={{ transitionDelay: `${visible ? 500 : 0}ms` }}
          >
            <div
              className="notif-ico"
              style={{
                background:
                  "linear-gradient(135deg, oklch(0.45 0.10 250), oklch(0.30 0.06 250))",
              }}
            >
              <Icon.Messages />
            </div>
            <div className="notif-body">
              <div className="notif-head">
                <span className="notif-app">信息 · 妈妈</span>
                <span className="notif-time">10:38</span>
              </div>
          ...
          <div
            className={`notif ${visible ? "in" : ""}`}
            style={{ transitionDelay: `${visible ? 580 : 0}ms` }}
          >
            <div
              className="notif-ico"
              style={{
                background:
                  "linear-gradient(135deg, oklch(0.50 0.14 25), oklch(0.32 0.08 25))",
              }}
            >
              <Icon.Calendar />
            </div>
            <div className="notif-body">
              <div className="notif-head">
                <span className="notif-app">日历 · 即将开始</span>
                <span className="notif-time">10:30</span>
              </div>
